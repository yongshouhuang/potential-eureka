...
| 类别 | 资产数（约） | 说明 |
|---|---:|---|
| 角色式神立绘 | 36 | 12 式神 ×（全身立绘 1 + 头像 1 + Q版 1） |
...
### 1.2 角色式神立绘（MVP 12 式神，可 10–15 浮动）
- **稀有度分布**：N×3 / R×4 / SR×3 / SSR×2（覆盖四档，保证卡框 R/SR/SSR 均被使用；N 复用通用框 + 素灰 tint）。
- **每式神三视图资产**：
...
5. MVP 式神数确认取 12（可在 10–15 浮动）及稀有度分布 N3/R4/SR3/SSR2。
