...
| **R3** | **MVP 式神数/稀有度分布待确认**：资产按 13 式神（N3/R4/SR3/SSR3）铺，可在 10–15 浮动 | ✅ 定稿 | 定稿 **13 = N3/R4/SR3/SSR3**，10–15 浮动（朱雀确认为火 SSR，放宽原 SSR2 锁）；分布已保证 R/SR/SSR 卡框均被使用。S3 已补齐至 13 张（含 `ssr_zhu_que` 火 SSR） |
...
2. ✅ **R3（定稿）**：MVP 式神数 = **13（N3/R4/SR3/SSR3）**，允许 10–15 浮动（朱雀确认为火 SSR，原 SSR2 锁放宽至 SSR3）。S3 已补齐至 13 张。
