...
> 评审对象：S2 门禁遗留 3 项设计/机制缺口（B-1 觉醒改写 / B-2 连携发射 / B-3 玩家选技）+ S3 范围（E5 Demo 串接、式神资产 8→12）
...
| 5 | 式神资产补齐 8→12 设计配合 | **PASS** | N3/R4/SR3/SSR2 分布下，4 新卡归属正好补全 5 个羁绊组（jian_zong/tie_bi/yu_zu/long_zu/hu_zu）且补齐玩家侧火行（灼烧归属） |
...
- 式神资产补齐方案在不破坏 R3（N3/R4/SR3/SSR2）与 5 个羁绊组完整性的前提下，同时解决了「玩家侧无火行 → 灼烧机制无归属」的隐藏缺口（见 §5.3）。
- **唯一待拍板项（非阻塞）**：概念文档 B3③ 举例「火系 SSR 觉醒灼烧」，但 R3 锁定的 SSR2（青龙/白虎）均为金行，无火系 SSR。本定稿将灼烧归属落到**新增火系 SR「朱雀」**——保持 R3 固定、仍完整交付灼烧机制。建议主理人确认「灼烧归属火 SR」可接受（见 §5.3 偏差说明），或调整 R3 引入火 SSR（会破坏已定稿分布，不推荐）。
...
S3 范围（据 `phase4-assembly.md §3`）：E5 全 4 故事（Demo 串接）+ E6-S5（AccessibilitySettings）+ E6-S6（MotionScale + CVD）+ 双端验证 + 式神资产补齐（8→12）。
...
- `shikigami_defs.json` 当前 **8 张**：N2（草隶/山童）、R2（铁甲/青羽）、SR2（幽冥/玄风）、SSR2（青龙/白虎）。
- R3 定稿（不可动）：**12 = N3 / R4 / SR3 / SSR2**，允许 10–15 浮动。
- 须补：**N+1 / R+2 / SR+1 = 4 张**。
...
- 4 张新卡须让 **5 个羁绊组各 ≥2 名成员**（否则空组），且**不改动已锁定 SSR2**（青龙/白虎，S2 数据与测试依赖）。
...
|---|---|---|---|---|---|---|
| `sr_zhu_que` | 朱雀 | **SR** | **火** | `[yu_zu]` | **yu_zu**（青羽+朱雀=2） | **灼烧归属**（火觉醒 → burn）；同时补玩家侧火行 |
...
| 稀有度 | 数量 | 式神（元素 / 羁绊组） |
|---|---|---|
| SSR | 2 | 青龙（金 / jian_zong+long_zu）、白虎（金 / jian_zong+hu_zu） |
| SR | 3 | 幽冥（木 / jian_zong）、玄风（木 / jian_zong）、**朱雀（火 / yu_zu）** |
...
**偏差说明（待主理人拍板）**：概念文档 `02a §B3④` 举例「火系 SSR 觉醒灼烧」，但 R3 锁定的 SSR2（青龙/白虎）均为金行、不可改（S2 数据/测试依赖）。本方案将灼烧归属落到**火系 SR 朱雀**，保持 R3 固定、完整交付灼烧机制。**建议主理人确认此归属可接受**；若坚持「火系 SSR」，须调整 R3 引入火 SSR（破坏已定稿分布与 S2 数据，不推荐）。
...
S3 三大 S2 遗留缺口（B-1/B-2/B-3）与式神资产补齐（8→12）均已给出**完整、数据驱动、可实现**的设计定稿；红线（R1–R5 / 支柱）经自检无违反；E5 Demo 的「好玩」维度已落到可量化 Playtest 口径。设计侧**不再阻塞 S3 开工**。
...
1. **（建议接受）灼烧归属火系 SR 朱雀**，而非概念文档举例的「火系 SSR」（R3 锁定 SSR2 为金行不可改）。见 §5.3。
2. **（建议采纳推荐项）B-3 引入 `qi` 气资源门控**觉醒技，以增强策略表达；若 S3 工时紧可降级为无消耗/冷却。
3. **式神资产补齐 4 张归属**（§5.3）是否批准——尤其是朱雀为 SR（非 SSR）承载灼烧、虬龙/虎威为 R 补全龙/虎族、火灵为 N 补火行。
...
【一句话汇报（主理人）】S3 设计评审 **PASS（定稿就绪，1 项待拍板）**：S2 三缺口 B-1（灼烧叠层：层数上限3/DoT=层数×3%×目标最大HP/水克火压制+木克增益/与连携正交）/B-2（战斗场景在 start_battle 后调 BondManager.compute_combo，零跨 import 红线守住）/B-3（每单位基础+觉醒 1–2 主动技、选目标、技能 element 决定五行倍率、推荐 qi 门控）均已给出可落地 AC；式神 8→12 以「朱雀(SR火/yu_zu)+虬龙(R/long_zu)+虎威(R/hu_zu)+火灵(N火)」补全 5 羁绊组并补玩家侧火行（灼烧归属），E5 Demo 的 MDA 动态/心流/三支柱验收口径已量化；唯一待拍板为「灼烧归属火 SR 朱雀」（R3 锁 SSR2 为金行，概念文档火系 SSR 举例不可行）。
